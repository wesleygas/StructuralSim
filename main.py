...
matrix_g, matrix_cut, force_list, free_dict = utils.matrixG(
    dict_bars, len(node_data[0]["COORDINATES"]))

dis_matrix_cut = utils.gauss_rules(matrix_cut, force_list, 10e5, 10e-31)

dis_matrix_g = utils.expandDisplacementMatrix(
    free_dict, dis_matrix_cut, len(matrix_g))
...
utils.write_exit(dis_matrix_g, nodes, np.around(
    complete_load_list, decimals=5), list_bars)
